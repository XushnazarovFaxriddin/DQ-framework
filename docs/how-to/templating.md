...
target:
  query: |
    ...
    WHERE last_updated_at >= TIMESTAMP("{{ partition_start_iso }}")
      AND last_updated_at <  TIMESTAMP("{{ partition_end_iso }}") AND status = '{{ status_filter }}'
```

CLI

```sh
dqf --config-file sw_res_header_test --filetype yaml --vars env=prod run_label=batch1 hours_ago=2 status_filter=ACTIVE
```
